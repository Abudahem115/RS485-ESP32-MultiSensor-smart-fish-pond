// ================= MODBUS =================
// ESP32 as Master (read NH4 sensor) + Slave (respond to Samkoon HMI)
// Sensor: Ammonium (RK500-15)
...
bool safeModbusRead(ModbusMaster &sensor, uint16_t address, uint16_t quantity) {
  ...
  if (result == sensor.ku8MBSuccess) {
    ...
    sensorErrors++;
    if (sensorErrors % 5 == 0) {
      Serial.printf("⚠️  NH4 Sensor Error (Count: %d) - Code: 0x%02X\n",
                    sensorErrors, result);
    }
    return false;
  }
...
void setup() {
  ...
  // Initialize Modbus Master for NH4 Sensor
  ...
  Serial.println("╔════════════════════════════════════════╗");
  Serial.println("║       NH4 SENSOR SYSTEM STARTED        ║");
...
// ========================================================================
// Read Ammonium Sensor (RK500-15)
// ========================================================================
void readNH4Sensor() {
  if (safeModbusRead(NH4Sensor, 0x0000, 2)) {
...
void sendToBlynk() {
  ...
  Serial.println("📤 NH4 Data sent to Blynk");
}
...
void printToDebugger() {
  Serial.println("\n  ╔══════════════════════════════════════╗");
  Serial.println("  ║          NH4 SENSOR DATA             ║");
  Serial.println("  ╠══════════════════════════════════════╣");
