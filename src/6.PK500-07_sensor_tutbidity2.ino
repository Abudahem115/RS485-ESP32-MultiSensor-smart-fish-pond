// ================= MODBUS =================
// ESP32 as Master (read Turbidity2 sensor) + Slave (respond to Samkoon HMI)
// Sensor: Turbidity2 (PK500-07)
...
// Blynk Virtual Pins - Turbidity2 Sensor
#define VP_TURB2   V11
...
bool safeModbusRead(ModbusMaster &sensor, uint16_t address, uint16_t quantity) {
  ...
  if (result == sensor.ku8MBSuccess) {
    ...
    sensorErrors++;
    if (sensorErrors % 5 == 0) {
      Serial.printf("⚠️  Turbidity2 Sensor Error (Count: %d) - Code: 0x%02X\n",
                    sensorErrors, result);
    }
    return false;
  }
...
void setup() {
  ...
  // Initialize Modbus Master for Turbidity2 Sensor
  ...
  Serial.println("╔════════════════════════════════════════╗");
  Serial.println("║   TURBIDITY2 SENSOR SYSTEM STARTED     ║");
...
void loop() {
  ...
  // Handle Turbidity2 Sensor reading
  handleSensorReading();
}
...
// ========================================================================
// Read Turbidity2 Sensor (PK500-07)
// ========================================================================
void readTurb2Sensor() {
  if (safeModbusRead(Turb2Sensor, 0x0000, 2)) {
...
void sendToBlynk() {
  ...
  Serial.println("📤 Turbidity2 Data sent to Blynk");
}
...
void printToDebugger() {
  Serial.println("\n  ╔══════════════════════════════════════╗");
  Serial.println("  ║        TURBIDITY2 SENSOR DATA        ║");
  Serial.println("  ╠══════════════════════════════════════╣");
